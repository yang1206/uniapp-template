<template>
  <view
    ...
    :style="[
      { width: `${view_width}px`, height: `${view_height}px`, background: '#000000' },
    ]"
  >
    <view v-if="!url" class="flex flex-center py-24"
      ><text style="font-size: 24rpx; color: white">请选择选择图片才可操作</text>
    </view>
    <!-- #ifndef APP-NVUE -->
    <canvas
      id="imgid"
    ...
    <image
      ...
      :class="[moveable ? 'opacity-10' : 'ani opacity-5']"
    ...
    <view
      ...
      :style="{ width: `${view_width}px`, height: `${80}px` }"
    >
