...
function initGraphStyle (ctx, style) {
  ...
  if( ctx.setFillStyle){
	  ctx.setFillStyle(getColorFromRgbValue(fill))
  }else{
	  ctx.fillStyle = getColorFromRgbValue(fill)
  }
  if( ctx.setStrokeStyle){
  	  ctx.setStrokeStyle(getColorFromRgbValue(stroke))
  }else{
  	  ctx.strokeStyle = getColorFromRgbValue(stroke)
  }
  if( ctx.SetShadowColor){
  	  ctx.setShadowColor(getColorFromRgbValue(shadowColor))
  }else{
...
function initGradient (ctx, style,shape) {
  ...
  gradientStops.forEach((stop, i) => gradient.addColorStop(stop, gradientColor[i]))
	...
	let gradientWithR = gradientWith.substr(0,1).toUpperCase()+gradientWith.slice(1)
	if(ctx['set'+gradientWithR]){
		ctx['set'+gradientWithR](gradient)
	}else{
