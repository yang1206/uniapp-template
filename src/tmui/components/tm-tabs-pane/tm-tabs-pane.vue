<template>
  ...
  >
    ...
    >
      <view class="pr-32">
        <tm-icon
          v-if="sc_top > refreshJuli && reFresh != 2"
...
const selfIndex = computed(() =>
  tabsActiveCacheTabse.value.findIndex((el) => el.key == _pname.value)
);
...
const isShowRender = computed(() => {
  ...
  return (
    selfIndex.value >= activeIndex.value - 1 && selfIndex.value <= activeIndex.value + 1
  );
});
