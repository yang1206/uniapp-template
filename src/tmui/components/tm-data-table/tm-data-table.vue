<template>
  <view>
    <scroll-view
      ...
      :style="[
        defaultProps.height
          ? { height: `${defaultProps.height} ${defaultProps.unit}` }
          : '',
          ...
          >
            ...
            >
              <tm-icon
                ...
                @click="headerClick(item.key, item.sort)"
                :_class="
                  sortKeys[item.key] == 'asc' ||
                  sortKeys[item.key] == 'none' ||
                  !sortKeys[item.key]
                    ? ''
                    : 'opacity-6'
                "
      ...
      <view :style="{ paddingTop: `${defaultProps.headerHeight}${defaultProps.unit}` }">
        ...
        >
          <tm-sheet
            ...
            v-for="(col, colIndex) in columnsList"
...
function setColData() {
  ...
  if (defaultProps.value.width > colWidth) {
    cellWidth =
      parseFloat(
        String((defaultProps.value.width - colWidth) / (colNum - colWidthNum))
      ).toFixed(2) - 0;
  }
  //======= E 计算宽度 =========//
  _columns.value?.forEach((col: DataTableColumn, index: number) => {
    ...
    if (col?.headerProps) {
      ...
      col.headerProps._class =
        col.headerProps._class ?? `flex-col-center-${col.align || "center"}`;
      ...
      col.headerProps = {
        ...
        height:
          defaultProps.value.headerHeight -
          (defaultProps.value.unit == "rpx" ? 24 : uni.upx2px(24)),
    ...
    col._headerProps = {
      width:
        col.headerProps.width -
        (col.headerProps.unit == "rpx"
          ? col.headerProps.padding[0] * 2
          : uni.upx2px(col.headerProps.padding[0] * 2)),
      height:
        col.headerProps.height -
        (col.headerProps.unit == "rpx"
          ? col.headerProps.padding[1] * 2
          : uni.upx2px(col.headerProps.padding[1] * 2)),
    ...
    if (col?.cellProps) {
      ...
      col.cellProps._class =
        col.cellProps._class ?? `flex-col-center-${col.align || "center"}`;
      ...
      col.cellProps = {
        ...
        height:
          defaultProps.value.cellHeight > 0
            ? defaultProps.value.cellHeight -
              (defaultProps.value.unit == "rpx" ? 24 : uni.upx2px(24))
            : 0,
        width:
          (_parseWidth == 0 ? 44 : _parseWidth) -
          (defaultProps.value.unit == "rpx" ? 40 : uni.upx2px(40)),
        margin: [0, 0],
        padding: [10, 6],
        unit: defaultProps.value.unit,
      };
    ...
    col._cellProps = {
      width:
        col.cellProps.width -
        (col.cellProps.unit == "rpx"
          ? col.cellProps.padding[0] * 2
          : uni.upx2px(col.cellProps.padding[0] * 2)),
      height:
        col.cellProps.height > 0
          ? col.cellProps.height -
            (col.cellProps.unit == "rpx"
              ? col.cellProps.padding[1] * 2
              : uni.upx2px(col.cellProps.padding[1] * 2))
          : 0,
