...
import type { pagesCustomType, pagesType, tabBarType } from './interface'
...
PageJsonInit.pages.forEach((el: any) => {
  ...
  const bg = (el.style?.navigationBarBackgroundColor ?? PageJsonInit?.globalStyle?.navigationBarBackgroundColor ?? '#FFFFFF') || '#FFFFFF'
  const txtColor = (el.style?.navigationBarTextStyle ?? PageJsonInit?.globalStyle?.navigationBarTextStyle ?? 'black') || 'black'
  pages.push({
    path: el.path,
    custom: customType,
...
if (Array.isArray(PageJsonInit?.subPackages ?? null)) {
  PageJsonInit?.subPackages.forEach((el: any) => {
    ...
    el.pages.forEach((el2: any) => {
      const elany: any = el2
      const bg = (el2.style?.navigationBarBackgroundColor ?? PageJsonInit?.globalStyle?.navigationBarBackgroundColor ?? '#FFFFFF') || '#FFFFFF'
      const txtColor = (el2.style?.navigationBarTextStyle ?? PageJsonInit?.globalStyle?.navigationBarTextStyle ?? 'black') || 'black'
      pages.push({
